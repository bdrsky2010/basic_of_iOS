...
struct ContentView : View {
    ...
    var body: some View {
		...
		.sheet(isPresented: $viewModel.isShowingView) {
...
class Coordinator: NSObject {
	...
	func createTexture(drawing: CGImage?) -> SimpleMaterial {
		if let drawing = drawing {
			...
			material.metallic = .float(0) // 텍스쳐를 사용하여 재료 색상을 설정
			return material
		}
		
		// 드로잉이 nil이면 간단한 머티리얼 반환
